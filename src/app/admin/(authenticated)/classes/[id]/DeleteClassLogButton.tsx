...
export default function DeleteClassLogButton({
  classLogId,
}: {
  classLogId: string;
}) {
  ...
  async function handleDelete() {
    setDeleting(true);
    try {
      const res = await fetch(`/api/admin/class-logs/${classLogId}`, {
        method: "DELETE",
      });
      if (!res.ok) {
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-sm text-red-600">Delete this log?</span>
        <button
